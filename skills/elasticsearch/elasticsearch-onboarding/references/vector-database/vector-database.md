...
description:
  Guide for using Elasticsearch as a vector database for AI applications. Use when a developer wants to store and
  retrieve embeddings, integrate with LangChain/LlamaIndex, build similarity search, or use Elasticsearch as a vector
  store for their AI pipeline.
---
...
For very large datasets, consider **quantization** (int8) to reduce memory; Elasticsearch 8.x supports byte-sized
vectors.
